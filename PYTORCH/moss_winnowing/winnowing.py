...
def plagiarismCheck(file1, file2):
    ...
    str1 = toText(token1)
    # print("These are tokens for file 1", str1)
    token2 = tokenize(file2)
    str2 = toText(token2)
    # print("These are the tokens to file2", str2)
    ...
    # print("This is the hashlist of file 1", HL1)
    # print("This is the hashList of file 2", HL2)
    fpList1 = fingerprints(HL1)
    fpList2 = fingerprints(HL2)
    matched_hashes = []
    # print("This is the fingerprints for file 1", fpList1)
    # print("This is the fingerprints for file 2", fpList2)
    ...
    for i in fpList1:
        for j in fpList2:
            if i == j:   #fingerprints match
                ...
                flag = 0
                match_1 = HL1.index(i)   #index of matching fingerprints in hash list, k-grams list
                match_2 = HL2.index(i)
                newStart = kGrams1[match_1][2]   #start position of matched k-gram in cleaned up code
                newEnd = kGrams1[match_1][3]   #end position

                hash_params = []
                hash_params.append(i)
                for k in token1:
                ...
                if flag == 1:
                    ...
                    hash_params.append(startx)
                    hash_params.append(endx)

                newStart = kGrams2[match_2][2]
                newEnd = kGrams2[match_2][2]
                flag=0
                for k in token2:
                    if k[2] == newStart:
                        startx = k[1]
                    ...
                    if flag == 1:
                        hash_params.append(startx)
                        hash_params.append(endx)
                matched_hashes.append(hash_params)
