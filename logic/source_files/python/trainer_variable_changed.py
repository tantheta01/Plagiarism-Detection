...
class MyDeddy:
    def __init__(self, args):
        self.args = args
        self.loss1 = 10000000000
        self.loss2 = 1000000
        self.loss3 = 1000000
        self.loss4 = 10000000

        self.loss5 = 10000000000
        self.loss6 = 1000000
        ...
        self.m1 = self._init_m1()
        self.ww1 = self._init_ww1()
        self.trenny = self._init_trenny()
        self.bcht = self._init_bcht()
        self.butrrr = self._init_butrrr()
        self.dfndsk = self._load_mode_state_dict()

        self.dffdsfs = self._init_data_loader(self.args.train_file_list, self.trenny)
        self.hrvsa = self._init_data_loader(self.args.val_file_list, self.bcht)

        self.hksdef = self._init_visual_hksdef()
        self.mlc = self._init_mlc()
        self.co_attention = self._init_co_attention()
        self.sentence_model = self._init_sentence_model()
        self.tuends = self._init_tuends()
    ...
    def _init_m1(self):
        m1 = os.path.join(self.args.model_path, self.args.saved_model_name)

        if not os.path.exists(m1):
            os.makedirs(m1)

        m1 = os.path.join(m1, self._get_now())
    ...
    def _init_butrrr(self):
        with open(self.args.butrrr_path, 'rb') as f:
            butrrr = pickle.load(f)

        self.ww1.write("Vocab Size:{}\n".format(len(butrrr)))
    ...
    def _load_mode_state_dict(self):
        ...
        try:
            ...
            self.ww1.write("[Load Model-{} Succeed!]\n".format(self.args.load_model_path))
            self.ww1.write("Load From Epoch {}\n".format(model_state['epoch']))
            return model_state
        except Exception as err:
            self.ww1.write("[Load Model Failed] {}\n".format(err))
            return None

    def _init_visual_hksdef(self):
        ...
        try:
            ...
            self.ww1.write("[Load Visual Extractor Succeed!]\n")
        except Exception as err:
            self.ww1.write("[Load Model Failed] {}\n".format(err))
    ...
    def _init_mlc(self):
        model = MLC(classes=self.args.classes,
                    ...
                    fc_in_features=self.hksdef.out_features,
                    k=self.args.k)

        try:
            model_state = torch.load(self.args.load_mlc_model_path)
            model.load_state_dict(model_state['model'])
            self.ww1.write("[Load MLC Succeed!]\n")
        except Exception as err:
            self.ww1.write("[Load MLC Failed {}!]\n".format(err))
    ...
    def _init_co_attention(self):
        model = CoAttention(version=self.args.attention_version,
                            ...
                            visual_size=self.hksdef.out_features,
                            k=self.args.k,
        ...
        try:
            ...
            model.load_state_dict(model_state['model'])
            self.ww1.write("[Load Co-attention Succeed!]\n")
        except Exception as err:
            self.ww1.write("[Load Co-attention Failed {}!]\n".format(err))
    ...
    def _init_logger(self):
        logger = Logger(os.path.join(self.m1, 'logs'))
        return logger

    def _init_ww1(self):
        ww1 = open(os.path.join(self.m1, 'logs.txt'), 'w')
        return ww1
    ...
    def _save_model(self,
        ...
        def save_whole_model(_filename):
            self.ww1.write("Saved Model in {}\n".format(_filename))
            torch.save({'hksdef': self.hksdef.state_dict(),
                        'mlc': self.mlc.state_dict(),
                        'co_attention': self.co_attention.state_dict(),
                        'sentence_model': self.sentence_model.state_dict(),
                        'tuends': self.tuends.state_dict(),
                        'optimizer': self.optimizer.state_dict(),
                        'epoch': epoch_id},
                       os.path.join(self.m1, "{}".format(_filename)))
        ...
        if val_loss < self.loss1:
            file_name = "val_best_loss.pth.tar"
            save_whole_model(file_name)
            self.loss1 = val_loss

        if train_loss < self.loss5:
            file_name = "train_best_loss.pth.tar"
            save_whole_model(file_name)
            self.loss5 = train_loss

        # if val_tag_loss < self.loss2:
        #     save_part_model("hksdef.pth.tar", self.hksdef.state_dict())
        #     save_part_model("mlc.pth.tar", self.mlc.state_dict())
        #     self.loss2 = val_tag_loss
        #
        # if val_stop_loss < self.loss3:
        #     save_part_model("sentence.pth.tar", self.sentence_model.state_dict())
        #     self.loss3 = val_stop_loss
        #
        # if val_word_loss < self.loss4:
        #     save_part_model("word.pth.tar", self.tuends.state_dict())
        #     self.loss4 = val_word_loss


class LSTMDebugger(MyDeddy):
    def _init_(self, args):
        MyDeddy.__init__(self, args)
        self.args = args
    ...
    def _epoch_train(self):
        ...
        for i, (images, _, label, captions, prob) in enumerate(self.dffdsfs):
            batch_tag_loss, batch_stop_loss, batch_word_loss, batch_loss = 0, 0, 0, 0
            images = self._to_var(images)

            visual_features, avg_features = self.hksdef.forward(images)
            ...
            for sentence_index in range(captions.shape[1]):
                ...
                for word_index in range(1, captions.shape[2]):
                    words = self.tuends.forward(topic, context[:, sentence_index, :word_index])
            ...
            if self.args.clip > 0:
                ...
                torch.nn.utils.clip_grad_norm(self.tuends.parameters(), self.args.clip)
            self.optimizer.step()
    ...
    def _epoch_val(self):
        ...
        # self.tuends.eval()
        #
        # for i, (images, _, label, captions, prob) in enumerate(self.hrvsa):
        #     batch_tag_loss, batch_stop_loss, batch_word_loss, batch_loss = 0, 0, 0, 0
    ...
    def _init_sentence_model(self):
        ...
        try:
            ...
            model.load_state_dict(model_state['model'])
            self.ww1.write("[Load Sentence Model Succeed!\n")
        except Exception as err:
            self.ww1.write("[Load Sentence model Failed {}!]\n".format(err))
    ...
    def _init_tuends(self):
        model = WordLSTM(butrrr_size=len(self.butrrr),
                         embed_size=self.args.embed_size,
                         hidden_size=self.args.hidden_size,
        ...
        try:
            model_state = torch.load(self.args.load_tuends_path)
            model.load_state_dict(model_state['model'])
            self.ww1.write("[Load Word Model Succeed!\n")
        except Exception as err:
            self.ww1.write("[Load Word model Failed {}!]\n".format(err))
...
if __name__ == '__main__':
    ...
    # Path Argument
    parser.add_argument('--butrrr_path', type=str, default='./data/new_data/butrrr.pkl',
                        help='the path for butrrrulary object')
    ...
    parser.add_argument('--visual_trained', action='store_true', default=True,
                        help='Whether train visual hksdef or not')
    ...
    parser.add_argument('--load_tuends_path', type=str,
                        default='.')
